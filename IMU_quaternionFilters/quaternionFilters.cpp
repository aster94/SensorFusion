...
//without mag
//you have to pass the gyroscope value as radiants/s: Gyro * PI /180.0f
...
const float * getEulerAngles () { 
	//pitch
	EulerAngles[0] = -asin(2.0f * (q[1] * q[3] - q[0] * q[2]));
	EulerAngles[0] *= RAD_TO_DEG;
	
	//roll
	EulerAngles[1]  = atan2(2.0f * (q[0] * q[1] + q[2] * q[3]), q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3]);
	EulerAngles[1] *= RAD_TO_DEG;

	//yaw
	EulerAngles[2] = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]);
	EulerAngles[2] *= RAD_TO_DEG;

  // Declination Correction: if E subtract if W add
